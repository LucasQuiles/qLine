...
DEFAULT_THEME: dict[str, Any] = {
    ...
    "layout": {
        ...
        "line2": ["cpu", "cpu_temp", "memory", "gpu_util", "gpu_mem",
                  "gpu_temp", "gpu_power", "disk", "local_models", "agents", "tmux"],
    ...
    if sys.platform == "darwin":
        ...
        for script in [
            ...
            try:
                ...
                continue
    # 4. WSL: query Windows terminal via PowerShell (cached 5s)
    if _IS_WSL:
        cache_file = "/tmp/qline-termwidth.cache"
        try:
            st = os.stat(cache_file)
        ...
        try:
            ...
            pass
    # 5. tput cols (may return PTY width, not real terminal)
    try:
        result = subprocess.run(
...
def _collect_cpu_temp_linux(state: dict[str, Any]) -> bool:
    ...
    thermal_base = os.path.join(PROC_DIR.replace("/proc", "/sys"), "class", "thermal")
    if PROC_DIR != "/proc":
...
def _collect_cpu_temp_macos(state: dict[str, Any]) -> bool:
    """Read CPU temp via macOS powermetrics or osx-cpu-temp. Returns True on success."""
...
DEFAULT_LINE2 = ["cpu", "cpu_temp", "memory", "gpu_util", "gpu_mem",
                 "gpu_temp", "gpu_power", "disk", "local_models", "agents", "tmux"]
